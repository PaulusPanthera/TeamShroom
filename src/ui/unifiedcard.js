...
export function renderUnifiedCard({
  ...
  // Member cards must never set Pokédex semantics.
  // Global UI handlers rely on data-pokemon-key for navigation/selection.
  const keyAttr = (!isMember && safeKey) ? ` data-pokemon-key="${safeKey}"` : '';
