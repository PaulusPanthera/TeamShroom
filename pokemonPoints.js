...
// List of families per tier (all members of the same family get the same points)
const TIER_FAMILIES = {
  "Tier 6": [
  ...
  "Tier 3": [
    ["Bagon"], ["Barboach"], ["Cacnea"], ["Carvanha"], ["Chatot"], ["Corsola"], ["Cryogonal"], ["Darumaka"], ["Drifloon"], ["Finneon"],
    ["Houndour"], ["Illumise"], ["Luvdisc"], ["Maractus"], ["Munna"], ["Nincada"], ["Pawniard"], ["Ralts"], ["Remoraid"], ["Seviper"],
...
// Helper to flatten families and assign points
const POKEMON_POINTS = {};
Object.entries(TIER_FAMILIES).forEach(([tier, families]) => {
  const points = TIER_POINTS[tier];
  families.forEach(fam => {
    fam.forEach(name => {
      // Normalize: lowercase, remove symbols, replace gender, remove brackets, hyphens, apostrophes, dots, spaces
      let key = name
        .toLowerCase()
        .replace(/\[.*\]/g, "")
        .replace(/♀/g, "-f")
        ...
        .replace(/[- '\.’]/g, "")
        .trim();
      POKEMON_POINTS[key] = points;
    });
  });
});

// Example usage: POKEMON_POINTS["nidoran-f"] === 2
